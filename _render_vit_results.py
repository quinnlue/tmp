"""Render the ViT metasmoothness JSON(s) -> markdown tables + PNGs.

Phase A (`vit_metasmooth_results.json`): head-to-head tables, the per-cluster
menu ranking by S_hat, per-example S_hat vs h, plus ablation bars and a
smoothness-vs-accuracy scatter (the paper's Fig-4-style tradeoff).
...
def find(stage=None, metaparam=None):
    out = []
    for r in rows:
...
# ---- Head-to-head ---------------------------------------------------------- #
print("### ViT head-to-head (phi = held-out cross-entropy)\n")
print("| metaparam | routine | f0 | val_acc | S (Def1) down | S_hat (Def2) up |")
print("|---|---|---|---|---|---|")
...
# ---- Per-cluster menu, ranked by S_hat ------------------------------------- #
...
print("\n### ViT per-cluster routines ranked by S_hat (higher = smoother)\n")
print("| routine | stage | dirs | f0 | val_acc | S (Def1) down | S_hat (Def2) up |")
print("|---|---|---|---|---|---|---|")
for r in ranked:
...
pe = sorted(
    (r for r in rows if r["metaparam"] == "per_example" and r["routine"] == "smooth"),
    key=lambda r: r["h"],
)
if pe:
    print("\n### ViT per-example S_hat vs h (smooth routine)\n")
...
# ---- Phase B accuracy table (optional) ------------------------------------- #
if os.path.exists(PATH_B):
    with open(PATH_B) as fh:
        bblob = json.load(fh)
    by = {}
    for r in bblob["results"]:
        if r["routine"] not in by or r["test_acc"] > by[r["routine"]]["test_acc"]:
            by[r["routine"]] = r
    print("\n### ViT Phase B: best test accuracy per routine\n")
    print("| routine | best test_acc | @ lr | val_best |")
    print("|---|---|---|---|")
    for name, r in by.items():
        print(f"| {name} | {r['test_acc']:.4f} | {r['lr']:g} | {r['val_best_acc']:.4f} |")
...
def color(n):
    if n in ("smooth", "smooth_wide") or n.startswith("final/3") or n == "final/30":
        return "#2a9d8f"
    if n in ("relu", "cls_pool", "final/1"):
        return "#e76f51"
    return "#5a6472"
...
axes[0].bar(names, shat, color=colors)
