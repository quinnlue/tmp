...
VM scaling: raise `N_TRAIN`, `TRAIN.epochs`, `HEAD_DIRS`/`ABL_DIRS`/`PE_DIRS`/`WIDE_DIRS` etc. in `_run_metasmooth_vm.py` (env-var driven); keep `amp="off"`. Render with `python _render_vm.py metasmooth_vm_deep_results.json`. CIFAR data on the VM must come from an HF-parquet `.npz` cache (`_build_cifar_cache.py`) — the torchvision mirror is throttled to ~30 KB/s.
...
Artifacts on the laptop (repo root): `metasmooth_vm_results.json`, `metasmooth_vm_deep_results.json`, `metasmooth_vm_ranking_deep.png`, `metasmooth_vm_tradeoff_deep.png`, `train_smooth_vm_results.json`, `phase_c_balanced_results.json`, `phase_c_corrupt_results.json`, `phase_c_shift_results.json`.
